...
namespace Accessor.Planner.Domain.Interface
{
    public interface ISolicitationService : IBaseService<Solicitation>
    {
        ...
        Task CancelProvider(Guid providerId, Guid solicitationId, string reason, double value, DateTime solicitationEndDate);
        Task CancelAccessor(Guid userId, Guid solicitationId, string reason);
