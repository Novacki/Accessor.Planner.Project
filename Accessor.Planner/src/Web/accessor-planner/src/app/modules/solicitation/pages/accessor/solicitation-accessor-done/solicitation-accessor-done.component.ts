...
@Component({
  selector: 'app-solicitation-accessor-done',
  templateUrl: './solicitation-accessor-done.component.html',
  styleUrls: ['./solicitation-accessor-done.component.css']
})
export class SolicitationAccessorDoneComponent implements OnInit {
    ...
    if(this.solicitations && this.clients) {
      if(this.client) {
        return this.solicitations.map(solicitation => {
          return { id: solicitation.id, status: solicitationStatusLabel.get(solicitation.status), client: solicitation.client.name, solicitationHistories: TransformDataColumns.transformSolicitationHistoryColumns(solicitation.solicitationHistories, this.providers, this.clients), 
        ...
        return this.solicitations.map(solicitation => {
          return { id: solicitation.id, status: solicitationStatusLabel.get(solicitation.status), client: solicitation.client.name, accessor: this.getNameAcessorById(solicitation.accessorId), solicitationHistories: TransformDataColumns.transformSolicitationHistoryColumns(solicitation.solicitationHistories, this.providers, this.clients), 
  ...
  private setFilter(): void {
    ...
    this.filter = SelectProfile.selectFilterByProfile(StatusSolicitation.done, this.client, this.provider);
  }
